...
for index, row in df.iterrows():
 if row['OBS.NO.'] == 1: Visit = Visit +1 
 df.at[index, 'Visit'] = Visit #row['Age'] + 5 

print(df) #AFTER
df.to_csv('Ga1105h00_01032025_Visit.csv', index=False)

df2 = pd.read_csv('GA1105h00_01032025_Visit.csv')
print(df2['Visit'].dtype)

grouped_visit = df2.groupby('Visit')['WEIGHT'].max()
print(grouped_visit)
